...
kMacro.minRequiredVersion = '0.7.301';
kMacro.version = '0.7.301';
kMacro.versions = {};
					...
					bodyLabel = {
						...
						name = function()
							...
							if valid then
								...
								if insertions then
									return self:GetMacroBodyFinal(vMacro, insertions)	
								else
									return self:GetMacroBodyFinal(vMacro)
								end
							end
						end,
						width = 'full',
	...
	for index=1,#macro.inserts + 1 do
		objReturn['insertGroup'..index] = {
			...
			args = {
				...
				[('insert%sindex'):format(index)] = {
					name = ('%s'):format('Change Index'),
					type = 'input',
					desc = 'Change the index of this insert, "pushing" other inserts to either side if necessary.',
					order = index,
					set = function(info,value)
						self:Insert_ChangeIndex(macro, index, tonumber(value))
						self:Options_BuildMacroOptions()
					end,
					get = function(info)
