function kHunt:ColorizeSubstringInString(subject, substring, r, g, b)
	local t = {};
	...
	local sOut = '';
	local sColor = kHunt:RGBToHex(r*255,g*255,b*255);
	for i = 1, strlen(subject) do
		if t[i] == true then
...
function kHunt:GetPlayerCount(type)
	type = type or 'raid';
	-- Check if post-MoP
...
function kHunt:CreateTimer(f,t,r,...)
	local v, i;
	if type(f) == 'string' then
		kHunt:Debug("CreateTimer func "..f, 1)
	end
	if r then -- repeater
		v  = {id = kHunt:GetUniqueTimerId(), interval = t, func = f, rep = r, args = ...}
	else
		v  = {id = kHunt:GetUniqueTimerId(), time = GetTime() + t, func = f, rep = r, args = ...}
	end
	table.insert(self.timers, v);
end
function kHunt:Debug(...)
	local isDevLoaded = IsAddOnLoaded('_Dev');
	local prefix = label and 'kHuntDebug ' .. label .. ': ' or 'kHuntDebug: ';
	-- CHECK IF _DEV exists
	if kHunt.db.profile.debug.enabled then
		--if not threshold or threshold <= kHunt.db.profile.debug.threshold then
			if isDevLoaded then
				dump(prefix, ...);
			else
				kHunt:Print(ChatFrame1, ('%s%s'):format(prefix,...))			
			end
		--end
	end
end
function kHunt:OnUpdate(index, elapsed)
...
function kHunt:RGBToHex(r, g, b)
	r = r <= 255 and r >= 0 and r or 0
	g = g <= 255 and g >= 0 and g or 0
	b = b <= 255 and b >= 0 and b or 0
	return string.format("%02x%02x%02x", r, g, b)
end
function kHunt:SplitString(subject, delimiter)
	local result = { }
	local from  = 1
