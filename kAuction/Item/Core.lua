...
function kAuction:Item_CleanupWhitelistDropdownValues()
	...
	for iItem,vItem in pairs(self.db.profile.items.whiteList) do
		...
		for iTemp,vTemp in pairs(temp) do
			if vTemp == vItem then
				matchFound = true;
			end
		end
	...
	self.db.profile.items.whiteList = temp;
	table.sort(self.db.profile.items.whiteList);
end
...
function kAuction:Item_GetItemTypeWhitelistData(itemLink)
	...
	tooltip:SetHyperlink(itemLink)
	for i=2,tooltip:NumLines() do
		local textLeft = _G["kAuctionTooltipTextLeft"..i]:GetText()
		local textRight = _G["kAuctionTooltipTextRight"..i]:GetText()
...
function kAuction:Item_GetItemTokenData(item)	
	-- Check by item string (tokens etc)
	-- whitelistData return:
	-- {id=67423, auctionType=2, currentItemSlot=5}
   local formatString, itemId;
   local itemName, itemLink = GetItemInfo(item);
   local nameFormat = "%s of the %s %s";
   local suffix = {"Conqueror", "Protector", "Vanquisher"}
   local slots = {
      ["Belt"] = 6,
      ["Boots"] = 8,
      ["Bracers"] = 9,
   ...
   if not item or not itemName then return nil end
   itemId = kAuction:Item_GetItemIdFromItemLink(itemLink);
   -- Loop suffix then slots
   for iSuffix,vSuffix in pairs(suffix) do
      for iSlot,vSlot in pairs(slots) do
         formatString = string.format(nameFormat, iSlot, "%w+", vSuffix)
         if string.match(itemName, formatString) then return {id=itemId, auctionType=2, currentItemSlot=vSlot}; end
      end
   end
   return nil;
end
...
function kAuction:Item_GetEmptyPaperdollTextureOfItem(item)
	...
	for i,loc in pairs(self.const.items.ItemEquipLocs) do
		if loc.name == itemEquipLoc then
			...
			local _,texture = GetInventorySlotInfo(loc.slotName)
			return texture;
		end
	end
	return nil;
...
function kAuction:Item_GetInventoryItemMatchTable(matchItem)
	...
	if type(id) == "number" and id<20 then
		...
		if self.popout.SlotInfo[id].other then
			local equippedItemLink = GetInventoryItemLink("player", self.popout.SlotInfo[id].other);
