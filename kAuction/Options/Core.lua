...
local table, tinsert, tremove, wipe, sort = table, table.insert, table.remove, wipe, sort
...
kAuction.minRequiredVersion = '2.1.144';
kAuction.version = '2.1.144';
kAuction.versions = {};
...
kAuction.const.items.ItemEquipLocs = {
	...
	{name = "INVTYPE_RANGED", slotName = "MainHandSlot", slotNumber = 16, formattedName = "Main Hand",},
	{name = "INVTYPE_THROWN", slotName = "MainHandSlot", slotNumber = 16, formattedName = "Main Hand",},
	{name = "INVTYPE_RANGEDRIGHT", slotName = "MainHandSlot", slotNumber = 16, formattedName = "Main Hand",},
	{name = "INVTYPE_RELIC", slotName = "MainHandSlot", slotNumber = 16, formattedName = "Main Hand",},
...
function kAuction:Options_CreateCustomOptions()
					...
					args = {
						addName = {
							...
							set = function(info, id)
								if (id) then
									kAuction:WeakAuras_AddConfig(id)
								end
							end,
							order = 0,
							width = 'full',
						...
						select = {
							...
							type = 'select',
							values = function()
								return kAuction:WeakAuras_GetConfigList();
							end,
							style = 'dropdown',
							set = function(info,value)
								kAuction.db.profile.weakauras.settings.selectedIndex = value;
								-- Close frame
								local frame
								frame = _G['kAuctionWeakAuraEditor']
								if frame then frame.texteditor:CancelClose() end
							end,
							get = function(info)
								return kAuction.db.profile.weakauras.settings.selectedIndex;
							end,
							order = 1,
							width = "normal",
						},
						...
						delete = {
							...
							type = 'execute',
							func = function()
								if not kAuction.db.profile.weakauras.settings.selectedIndex or not (#kAuction.db.profile.weakauras.configs > 0) then return end
								local dialog = self:Gui_CreateDialog('WeakAuraConfigDelete',
									"|cFF"..kAuction:RGBToHex(0,255,0).."Confirm deletion?|r|n|nConfig ID: |cFF"..kAuction:RGBToHex(255,0,0)..kAuction.db.profile.weakauras.configs[kAuction.db.profile.weakauras.settings.selectedIndex].id.."|r|n")
								self:Gui_CreateDialogButton('Delete', 'Delete', dialog, function() 
									tremove(kAuction.db.profile.weakauras.configs, kAuction.db.profile.weakauras.settings.selectedIndex);
									kAuction.db.profile.weakauras.settings.selectedIndex = nil;
									-- Close frame
									local frame
									frame = _G['kAuctionWeakAuraEditor']
									if frame then frame.texteditor:CancelClose() end		
								...
								self:Gui_CreateDialogButton('Cancel', 'Cancel', dialog, function() dialog.Close() end)					
							end,
							order = 3,
							width = "half",
						},
						send = {
							name = 'Send',
							desc = 'Click to broadcast this configuration to other kAuction users.',
							type = 'execute',
							func = function()
								if not kAuction.db.profile.weakauras.settings.selectedIndex or not (#kAuction.db.profile.weakauras.configs > 0) then return end
								local dialog = self:Gui_CreateDialog('WeakAuraConfigSend',
									"|cFF"..kAuction:RGBToHex(0,255,0).."Confirm broadcast?|r|n|nConfig ID: |cFF"..kAuction:RGBToHex(255,0,0)..kAuction.db.profile.weakauras.configs[kAuction.db.profile.weakauras.settings.selectedIndex].id.."|r|n")
								self:Gui_CreateDialogButton('Send', 'Send', dialog, function() 
									if kAuction.db.profile.weakauras.configs[kAuction.db.profile.weakauras.settings.selectedIndex] and kAuction.db.profile.weakauras.configs[kAuction.db.profile.weakauras.settings.selectedIndex].id and kAuction.db.profile.weakauras.configs[kAuction.db.profile.weakauras.settings.selectedIndex].value then
										local config = kAuction.db.profile.weakauras.configs[kAuction.db.profile.weakauras.settings.selectedIndex];
										kAuction:SendCommunication("WeakAuraConfigBroadcast", kAuction:Serialize(config.id, config.value), 3);
									end
									dialog.Close()
								end)
								self:Gui_CreateDialogButton('Cancel', 'Cancel', dialog, function() dialog.Close() end)							
						...
						rename = {
							...
							func = function()
								if not kAuction.db.profile.weakauras.settings.selectedIndex or not (#kAuction.db.profile.weakauras.configs > 0) then return end
								local dialog = self:Gui_CreateDialog('WeakAuraConfigRename',
									"|cFF"..kAuction:RGBToHex(0,255,0).."Enter new ID for this config|r|n|nCurrent ID: |cFF"..kAuction:RGBToHex(255,0,0)..kAuction.db.profile.weakauras.configs[kAuction.db.profile.weakauras.settings.selectedIndex].id.."|r|n")
								self:Gui_CreateDialogButton('Rename', 'Rename', dialog, function() 
									if dialog.textbox:GetText() then
										kAuction.db.profile.weakauras.configs[kAuction.db.profile.weakauras.settings.selectedIndex].id = dialog.textbox:GetText();
										kAuction.dialog:SelectGroup('kAuction', "weakauras", "configs")
									end
									dialog.Close()
								end)
								self:Gui_CreateDialogButton('Cancel', 'Cancel', dialog, function() dialog.Close() end)	
								self:Gui_CreateDialogTextBox(dialog, kAuction.db.profile.weakauras.configs[kAuction.db.profile.weakauras.settings.selectedIndex].id)	
							end,
							order = 5,
							width = "half",
						},
