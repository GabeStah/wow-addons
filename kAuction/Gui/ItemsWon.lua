...
function kAuction:Gui_OnEnterItemsWonAnchorIcon(frame)
	local sTitle, sClickText, sText = self:Gui_GetItemsWonIconStrings('anchor')		
...
function kAuction:Gui_OnEnterItemsWonCloseIcon(frame)
	local sTitle, sClickText, sText = self:Gui_GetItemsWonIconStrings('close')		
	kAuction:Gui_SetItemsWonIconTooltipStrings(frame,sTitle,sText,sClickText)
end
...
function kAuction:Gui_UpdateItemsWonIcons(frame, match)
	...
	if matches and #matches > 0 then
		...
		for line=1,itemFrameCount do
			...
			if lineplusoffset <= #matches then
				...
				kAuction:Gui_UpdateItemsWonIcons(itemFrame,matches[lineplusoffset])
				itemFrame:Show();
			else
				itemFrame:Hide();
			end
