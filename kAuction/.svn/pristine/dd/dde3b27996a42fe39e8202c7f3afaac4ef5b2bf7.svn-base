...
function kAuction:OnInitialize()
	...
	-- Create options
	kAuction:Options_CreateCustomOptions();	
	...
	self.updateFrame:SetScript("OnUpdate", function(frame,elapsed) kAuction:OnUpdate(1, elapsed) end)
	-- Init council list
...
function kAuction:RegisterLibSharedMediaObjects()
	...
	-- Nyan
	sharedMedia:Register("sound", "Wut", [[Interface\AddOns\kAuction\Sounds\wut.mp3]]);
	-- Icons
	...
	-- Ogg
	sharedMedia:Register("sound", "Penguin", [[Interface\AddOns\kAuction\Sounds\penguin.ogg]]);
	sharedMedia:Register("sound", "Penguin 2", [[Interface\AddOns\kAuction\Sounds\penguin2.ogg]]);
	sharedMedia:Register("sound", "PenguinStep", [[Interface\AddOns\kAuction\Sounds\penguinstep.ogg]]);
end

function kAuction:OnEnable()
	if kAuction:Client_IsServer() then
		if kAuction:GetPlayerCount() > 0 then
			self.isInRaid = true;
	...
	kAuction:Gui_EncounterJournal_Initialize()
end
...
function kAuction:LOOT_OPENED()
	...
	self.isLooting = true;
	if self.enabled and self.isActiveRaid and kAuction:GetPlayerCount() > 0 and kAuction:Client_IsServer() then -- Player in raid and raid leader
		...
		if kAuction:Server_HasCorpseBeenAuctioned(guid) == false then -- Check if corpse auctioned already.
			...
			for i = 1, GetNumLootItems() do
				if (LootSlotHasItem(i)) then
					kAuction:Debug('EVENT: LOOT_OPENED' .. ' - Get items', 3)
...
function kAuction:RAID_ROSTER_UPDATE()
	...
	if self.isInRaid == false and kAuction:GetPlayerCount() > 0 then
	...
	elseif self.isInRaid == true and isActiveRaid == true and kAuction:GetPlayerCount() == 0 then
		self.hasRunVersionCheck = false;
		self.isInRaid = false;
		-- Check if Server
		if kAuction:Client_IsServer() then
	...
	if success then
		if prefix == "kAuction" and distribution == "RAID" then
			...
			if (command == "WeakAuraConfigBroadcast") then
				kAuction:WeakAuras_ConfigRecieved(sender, data);
			end
			-- Refresh frames
...
function kAuction:Debug(value, threshold, label)
	local isDevLoaded = IsAddOnLoaded('_Dev');
	local prefix = label and 'kAuctionDebug ' .. label .. ': ' or 'kAuctionDebug: ';
	-- CHECK IF _DEV exists
	if kAuction.db.profile.debug.enabled then
		if not threshold or threshold <= kAuction.db.profile.debug.threshold then
			if isDevLoaded then
				dump(prefix, value);
			else
				if type(value) == 'table' then print(prefix,value) else
					kAuction:Print(ChatFrame1, ('%s%s'):format(prefix,value))			
				end		
			end
		end
	end
...
function kAuction:GetPlayerCount(type)
	return GetNumGroupMembers()
end
function kAuction:IsRaidLeader(unit)
	unit = unit or 'player'
	return (UnitIsGroupLeader('player') and IsInRaid())
end