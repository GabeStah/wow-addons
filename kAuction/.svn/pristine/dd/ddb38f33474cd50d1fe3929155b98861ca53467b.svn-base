				...
				if wishlistItem then
					if wishlistItem.autoBid == true then
						...
						if wishlistItem.alert == true then
							...
							local dialog = self:Gui_CreateDialog('PromptAutoBid',
								'|cFF'..kAuction:RGBToHex(0,255,0)..'kAuction|r|n|nAn automatic '..sBidType..' bid has been entered for the recently created auction of '..auction.itemLink..' due to your wishlist of this item.|n|nWould you like to keep or cancel your bid?')
							self:Gui_CreateDialogButton('KeepBid', 'Keep Bid', dialog, function() dialog.Close() end)
							self:Gui_CreateDialogButton('Cancel', 'Cancel', dialog, function() kAuction:Gui_OnClickAuctionBidButton(auction, 'none') dialog.Close() end, _, {r=1,g=0,b=0,a=1})
						end
					elseif wishlistItem.alert == true then
						...
						local dialog = self:Gui_CreateDialog('WishlistBid',"|cFF"..kAuction:RGBToHex(0,255,0).."kAuction|r|n|n"..
						...
						self:Gui_CreateDialogButton('Yes', 'Yes', dialog, function() kAuction:Gui_OnClickAuctionBidButton(auction, wishlistItem.bidType, wishlistItem.bestInSlot, wishlistItem.setBonus) dialog.Close() end)
						self:Gui_CreateDialogButton('No', 'No', dialog, function() dialog.Close() end, _, {r=1,g=0,b=0,a=1})						
					end
				end
			end
		end
...
function kAuction:Client_DataUpdateReceived(sender, data)
	...
	if type == "auction" then
		local index = kAuction:Client_GetAuctionIndexByAuctionId(auction.id);
...
function kAuction:Client_IsServer()
	...
	-- Verify raid leader
	if self:GetPlayerCount() > 0 and self:IsRaidLeader() then -- Current Server
		self.isServer = true;
		return true;
	elseif self:GetPlayerCount() == 0 and self.isServer then -- Previous Server
		return true;
	end
	return false;
end
...
function kAuction:Client_VersionInvalidReceived(sender, data)
	if not self.hasRunVersionCheck then
		...
		local dialog = self:Gui_CreateDialog(
			'VersionInvalid',
			"|cFF"..kAuction:RGBToHex(0,255,0).."kAuction out of date.|r|n|nYour version: |cFF"..kAuction:RGBToHex(255,0,0)..self.version.."|r|nRequired version: |cFF"..kAuction:RGBToHex(255,255,0)..minRequiredVersion.."|r|nServer version: |cFF"..kAuction:RGBToHex(0,255,0)..serverVersion.."|r|n|nPlease exit World of Warcraft and update your latest version from:|n|cFF"..kAuction:RGBToHex(190,0,110).."wow.curseforge.com/projects/kauction|r")
		self:Gui_CreateDialogButton('OK', 'OK', dialog, function() dialog.Close() end)
...
function kAuction:Client_WeakAuraConfigReceived(sender, data)
