...
local FauxScrollFrame_GetOffset, FauxScrollFrame_Update, EasyMenu = FauxScrollFrame_GetOffset, FauxScrollFrame_Update, EasyMenu
-- Author      : Gabe
	...
	if button == "LeftButton" then
		...
		elseif IsShiftKeyDown() then
			...
			if self.auctions[offset+row].bids and #self.auctions[offset+row].bids > 0 then
				...
				if self.auctions[offset+row].visiblePublicDetails or kAuction:Client_IsServer() or kAuction:IsLootCouncilMember(self.auctions[offset+row]) then
...
function kAuction:Gui_CreateDialog(id, text, width, height, margin, color)
   id = id or 'Default'
   width = width or 300
   height = height or 150
   color = color or {r=0,g=0,b=0,a=0.8}
   frameName = ('%s%s'):format('kAuctionDialog', id)
...
function kAuction:Gui_CreateDialogButton(id, text, parent, onClick, height, color, hoverColor)
   id = id or 'Button'
   text = text or 'Yes'
   height = height or 30
...
function kAuction:Gui_CreateDialogTextBox(parent, text, highlightText)
   text = text or ''
...
function kAuction:Gui_GetFrameIndexOfAuction(auction)
	local line;
	for line=1,5 do
		local lineplusoffset = line + FauxScrollFrame_GetOffset(kAuctionMainFrameMainScrollContainerScrollFrame);
...
function kAuction:Gui_GetBidFrameIndexOfName(name)
	local line;
	for line=1,5 do
		local lineplusoffset = line + FauxScrollFrame_GetOffset(kAuctionMainFrameBidScrollContainerScrollFrame);
		if lineplusoffset <= #(self.auctions) then
			if self.auctions[lineplusoffset] then
				return line;
			end
...
function kAuction:Gui_InitializeFrames()
	...
	-- Editor
	kAuction:WeakAuras_CreateEditorFrame()
end
...
function kAuction:Gui_MinimizeFrame()
	kAuction:ShrinkFrame(self.gui.main_frame, self.db.profile.gui.mainframe.anchorpoint, minwidth, minheight);
	self.gui.main_frame:SetHeight(minheight);
	self.gui.main_frame:SetWidth(minwidth);
	_G["kAuction_MainFrameTitle".."Minimize"]:Hide();
	_G["kAuction_MainFrameTitle".."Maximize"]:Show();
	self.gui.auctions_frame:Hide();
	self.gui.title_bar_text:SetText("kAuction");
end
-- TODO: UPDATE
function kAuction:Gui_MaximizeFrame()
	...
	self.gui.main_frame:SetHeight(maxheight);
	self.gui.main_frame:SetWidth(maxwidth);
	_G["kAuction_MainFrameTitle".."Minimize"]:Show();
	_G["kAuction_MainFrameTitle".."Maximize"]:Hide();
	self.gui.auctions_frame:Show();
	self.gui.title_bar_text:SetText("kAuction " .. kAuction_VERSION);
