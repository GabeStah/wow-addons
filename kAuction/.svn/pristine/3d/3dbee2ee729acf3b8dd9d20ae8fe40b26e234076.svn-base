...
function kAuction:WeakAuras_ConfigRecieved(sender, data)
	...
	if not index then 
		...
		local dialog = self:Gui_CreateDialog('WeakAuraConfigReceived',
			"|cFF"..kAuction:RGBToHex(155,155,0)..sender.."|r|cFF"..kAuction:RGBToHex(0,255,0).." sent you an updated WeakAuras configuration.  Accept this update?|r|n|nConfig ID: |cFF"..kAuction:RGBToHex(255,0,0)..id.."|r|n")
		self:Gui_CreateDialogButton('Accept', 'Accept', dialog, function() 
			kAuction:WeakAuras_UpdateConfig(id, value)
			kAuction.dialog:SelectGroup('kAuction', "weakauras", "configs")
			dialog.Close()
		end)
		self:Gui_CreateDialogButton('Deny', 'Deny', dialog, function() dialog.Close() end)
	end
end
