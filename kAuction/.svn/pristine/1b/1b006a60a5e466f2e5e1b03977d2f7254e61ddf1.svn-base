...
function kAuction:WishlistGui_RefreshMainFrame(initialLoad)
	if not self.gui.frames.list.main then return end
...
function kAuction:WishlistGui_CreateWidget_ListTree(parent)
	...
	self.gui.frames.list.tree:SetCallback('OnGroupSelected', function(widget,event,value,d,e)
		...
		if s2 == 'delete' then
			local dialog = self:Gui_CreateDialog('WishlistDelete',
				"|cFF"..kAuction:RGBToHex(0,255,0).."kAuction|r|nDo you really wish to delete the |cFF"..kAuction:RGBToHex(0,255,0).. kAuction:Wishlist_GetNameById(tonumber(s3)) .."|r wishlist?")
		...
		elseif s2 == 'rename' then
			local dialog = self:Gui_CreateDialog('WishlistRename',
				"Enter the new name for the |cFF"..kAuction:RGBToHex(0,255,0).. kAuction:Wishlist_GetNameById(tonumber(s3)) .."|r wishlist below.")
			self:Gui_CreateDialogButton('Rename', 'Rename', dialog, function() 
				kAuction:Wishlist_SetListFlag(tonumber(s3), 'name',  dialog.textbox:GetText());
				kAuction:WishlistGui_RefreshMainFrame();
				dialog.Close()
			end)
			self:Gui_CreateDialogButton('Cancel', 'Cancel', dialog, function() dialog.Close() end)	
			self:Gui_CreateDialogTextBox(dialog, kAuction:Wishlist_GetNameById(tonumber(s3)))
		elseif s2 == 'addmanual' then
			local dialog = self:Gui_CreateDialog('WishlistManualItem',
				"To manually add an item not found in your local cache, you must have the item id and item name.|n"..
				"Enter the exact item id and item name (capitalization and punctuation required), seperated by a semicolon (;) or colon (:).|n|n"..
				"Example:|cFF"..kAuction:RGBToHex(0,255,0).. "12784;Arcanite Reaper|r",500,250)
			self:Gui_CreateDialogButton('Add', 'Add Item', dialog, function() 
				if dialog.textbox:GetText() then
					local sFirst, sSecond = strsplit(';', dialog.textbox:GetText());
			...
			self:Gui_CreateDialogButton('Cancel', 'Cancel', dialog, function() dialog.Close() end)	
			self:Gui_CreateDialogTextBox(dialog)
		end
		if s3 then
