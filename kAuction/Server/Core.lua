...
function kAuction:Server_AwardAuction(auction, winner)
	...
	if auction.winner or auction.disenchant then
		...
		if self.db.profile.looting.autoAssignIfMasterLoot and self:IsRaidLeader() and (lootMethod=='master' and masterLooterId==0) and self.isLooting and corpseGuid == auction.corpseGuid and auction.looted == false then
			...
			if auction.winner then
				...
				for ci = 1, kAuction:GetPlayerCount() do
...
function kAuction:Server_WhisperAuctionToRaidRoster(itemLink)
	...
	for i = 1, kAuction:GetPlayerCount() do
...
function kAuction:Server_AuctionItem(id, corpseGuid, corpseName)
	...
	kAuction:Debug(("Server_AuctionItem %s."):format(id or ''), 1);
	...
	local whitelistData = kAuction:Item_GetItemWhitelistData(itemLink) or kAuction:Item_GetItemTokenData(itemLink) or kAuction:Item_GetItemTypeWhitelistData(itemLink) or {};
	--[[
			...
			if wishlistItem then
				if wishlistItem.autoBid == true then
					...
					if wishlistItem.alert == true then
						...
						local dialog = self:Gui_CreateDialog(
							'KeepBid',
							"|cFF"..kAuction:RGBToHex(0,255,0).."kAuction|r|n|n"..
						...
						self:Gui_CreateDialogButton('Keep', 'Keep Bid', dialog, function() dialog.Close() end)
						self:Gui_CreateDialogButton('Cancel', 'Cancel Bid', dialog, function() kAuction:Gui_OnClickAuctionBidButton(self.auctions[#self.auctions], 'none') dialog.Close() end)
					end
				elseif wishlistItem.alert == true then
					...
					local dialog = self:Gui_CreateDialog('AutoBid',
						"|cFF"..kAuction:RGBToHex(0,255,0).."kAuction|r|n|n"..
					...
					self:Gui_CreateDialogButton('Yes', 'Yes', dialog, function() kAuction:Gui_OnClickAuctionBidButton(self.auctions[#self.auctions], wishlistItem.bidType, wishlistItem.bestInSlot, wishlistItem.setBonus) dialog.Close() end)
					self:Gui_CreateDialogButton('No', 'No', dialog, function() dialog.Close() end)
				end
			end
		end
	end
...
function kAuction:Server_GetLootCouncilMemberCount()
	...
	for i,member in pairs(self.db.profile.looting.councilMembers) do
		for iR = 1, kAuction:GetPlayerCount() do
...
function kAuction:Server_ConfirmStartRaidTracking()
	...
	local dialog = self:Gui_CreateDialog('StartRaidTracking',
		"|cFF"..kAuction:RGBToHex(0,255,0).."kAuction|r|nDo you wish to start tracking this raid?")
	self:Gui_CreateDialogButton('Yes', 'Yes', dialog, function() kAuction:Server_StartRaidTracking() dialog.Close() end)
	self:Gui_CreateDialogButton('No', 'No', dialog, function() dialog.Close() end)						
end
function kAuction:Server_ConfirmStopRaidTracking()
	...
	local dialog = self:Gui_CreateDialog('StopRaidTracking',
		"|cFF"..kAuction:RGBToHex(0,255,0).."kAuction|r|nDo you wish to stop tracking this raid?")
	self:Gui_CreateDialogButton('Yes', 'Yes', dialog, function() kAuction:Server_StopRaidTracking() dialog.Close() end)
	self:Gui_CreateDialogButton('No', 'No', dialog, function() dialog.Close() end)						
end
...
function kAuction:Server_StopRaidTracking()
	...
	local dialog = self:Gui_CreateDialog('GetRaidXML',
		"|cFF"..kAuction:RGBToHex(0,255,0).."kAuction|r|nCopy raid Xml export string below.")
	self:Gui_CreateDialogButton('Done', 'Done', dialog, function() dialog.Close() end)
	self:Gui_CreateDialogTextBox(dialog, kAuction:Server_GetRaidXmlString(), true)
