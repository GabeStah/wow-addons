...
function kLoot:View_ScrollItem_CurrentItemFrames_Create(parent, name, level, test)
	...
	nameFrame:addEvent('OnMouseWheel', function(delta)
		kLoot:Debug('nameFrame, getOffset', HybridScrollFrame_GetOffset(parent:GetParent()), 2)
		if delta == 1 then
			HybridScrollFrame_SetOffset(parent:GetParent(), HybridScrollFrame_GetOffset(parent:GetParent()) + 1)
		elseif delta == -1 then
			HybridScrollFrame_SetOffset(parent:GetParent(), HybridScrollFrame_GetOffset(parent:GetParent()) - 1)
		end
		kLoot:Debug('nameFrame, post change getOffset', HybridScrollFrame_GetOffset(parent:GetParent()), 2)
		kLoot:Debug('nameFrame, onMouseWheel', delta, 2)
		-- Redraw
		kLoot:View_ScrollItem_CurrentItem_Update(parent, parent:GetParent().currentItemTestData)
