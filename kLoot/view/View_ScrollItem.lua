...
function kLoot:View_ScrollItem_Align(frame)
...
function kLoot:View_ScrollItem_Create(name, parent, width, height, defaultColor)
	...
	frame.getScrollItemIndex = function()
		...
		return kLoot:View_GetFlag(frame, 'scrollItemIndex')
	end	
...
function kLoot:View_ScrollItem_GetScrollItemCount(parent, objectType)
	if not parent or not parent:GetNumChildren() then return end
	local count
	for i,v in ipairs({parent:GetChildren()}) do
...
function kLoot:View_ScrollItem_SetScrollItemIndex(frame, parent)
