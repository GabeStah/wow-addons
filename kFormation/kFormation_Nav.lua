...
--[[ Nav_GetMapEffectivePositionFromCoords
PURPOSE: Get the x,y position for map frame at given coordinates
ARGS:	xCoord [number] - x coordinate
		yCoord [number] - y coordinate
		mapFrame [frame] - [Optional] Map frame with which to check for position
RETURN:	x [number] - X position
		y [number] - Y position
]]
function kFormation:Nav_GetMapEffectivePositionFromCoords(xCoord, yCoord, mapFrame)
	local fMap = getglobal(kFormation.const.gui.frames.mapName);
	if mapFrame then
		fMap = mapFrame;
	end
	local x = fMap:GetWidth() * xCoord;
	local y = fMap:GetHeight() * yCoord;
	return x, y;
end